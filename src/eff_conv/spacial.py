...
class SimilarityFunction:
    ...
    def calculate_gradients(
        referent_locations: np.ndarray, word_locations: np.ndarray
    ) -> np.ndarray:
        """
        ...
        raise NotImplementedError(
            "SimilarityFunction is an abstract class, do not call calculate_gradients"
        )
...
class CosineSimilarity(SimilarityFunction):
    ...
    def calculate_gradients(
        ...
        projections = (
            referent_locations[:, None, :]
            * (dot_products / (referent_norms**2)[:, None])[:, :, None]
        )
...
def run_grad_step(
    ...
    if (
        len(referent_locations.shape) != 2
        or len(word_locations.shape) != 2
        or len(priors.shape) != 2
    ):
    ...
    if dimensionality != word_locations.shape[1]:
        raise ValueError(
            f"Conflicting dimensionality arguments: {dimensionality} and {word_locations.shape[1]}"
        )
    if ref_num != priors.shape[0]:
        raise ValueError(
            f"Confliction referent numbers: {ref_num} and {priors.shape[0]}"
        )
    if word_nums != priors.shape[1]:
        raise ValueError(
            f"Confliction word_nums numbers: {word_nums} and {priors.shape[1]}"
        )

    # Gradients for every referent and word combination, weighted by priors
    # Is ||U|| x ||W|| x L
    weighted_gradients = (
        sim_func.calculate_gradients(referent_locations, word_locations)
        * priors[:, :, None]
    )
